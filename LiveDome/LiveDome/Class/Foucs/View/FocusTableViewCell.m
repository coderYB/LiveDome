...
- (void)setPlayerModel:(FocusModel *)playerModel{
    _playerModel = playerModel;
    _nameLabel.text = playerModel.nickname;
    if ([playerModel.position isEqualToString:@""]) {
        [_address setTitle:@"难道在火星?" forState:(UIControlStateNormal)];
    }else{
        [_address setTitle:playerModel.position forState:(UIControlStateNormal)];
    }
    [_iconImage sd_setImageWithURL:[NSURL URLWithString:playerModel.photo]];
    [_coverImage sd_setImageWithURL:[NSURL URLWithString:playerModel.photo] placeholderImage:[UIImage imageNamed:@"liveRoom"]];
    _peopleNumber.text = [NSString stringWithFormat:@"%zd人在看",playerModel.allnum];
}
@end
